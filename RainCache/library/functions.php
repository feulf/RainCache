<?php
    ...
    function RainCacheLoad( $config ) {
        ...
        if ($config['cache']) {
            ...
            $config['cache_filepath'] = $config['absolute_cache_dir'] . $config['prefix'] . md5($_SERVER['REQUEST_URI'] . serialize($container) ) . '.html';

            // if the cache is valid draw the cache and exit
            if (file_exists($config['cache_filepath']) && (time() - filemtime($config['cache_filepath']) < $config['cache_expiration'] )) {                
